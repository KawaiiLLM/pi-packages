...
- `pi-usage` 本地日志金额改为一个进程内增量账本，今日、5h、周窗口共享单轮分块读取；按文件字节位置、尾部指纹和 EOF 临时行跟踪追加，用回复哈希及文件引用撤回分叉、重置与确认删除。重置完成后原子替换，普通追加逐块提交；尾指纹独立复制为小缓冲，临时 IO 失败沿用旧贡献，进程退出不落盘。并发刷新排队合并下一轮，调用者取消不停止共享扫描；仅仍登记的用途自身起点后移才触发清理，注销及重新登记不清理；等待中的旧起点在交付前不会被清掉，回退到已清理区域重建。会话停止仅取消旧读取和等待，重启沿用同一账本与游标，并在旧读取结束后开始新读取；快照代次隔离旧异步结果。公开 snapshot 和计费口径保持不变；真实会话目录注销／重新登记序列及三个窗口金额对照通过，完整性能与线上卡顿验收仍未完成。
