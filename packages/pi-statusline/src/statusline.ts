...
import { subscribeUsageSnapshots, usageModelKey } from "@narumitw/pi-usage/snapshot";
...
export default function statusline(pi: ExtensionAPI) {
	...
	const clearUsage = () => {
		...
		runtime.dailySpent = undefined;
		runtime.dailyBudgetPercent = undefined;
		runtime.fast = undefined;
	};
	...
	const installFooter = (ctx: ExtensionContext) => {
		...
		unsubscribeUsage = subscribeUsageSnapshots(pi.events, (snapshot) => {
			if (
				generation !== sessionGeneration ||
				...
				return;
			runtime.usage = snapshot.usage;
			runtime.dailySpent = snapshot.dailySpend;
			runtime.dailyBudgetPercent = snapshot.dailyBudgetPercent;
			runtime.fast = snapshot.fast;
			refresh();
		});
	...
	pi.on("model_select", (_event, ctx) => {
		if (ownsRuntime(ctx)) installFooter(ctx);
		refresh();
	});
