...
import { USAGE_SNAPSHOT_EVENT, usageModelKey } from "@narumitw/pi-usage/snapshot";
...
test("balanced footer fits common widths and keeps context visible at narrow widths", async () => {
	...
	try {
		...
		mock.eventBus.emit(USAGE_SNAPSHOT_EVENT, {
			sessionId: ctx.sessionManager.getSessionId(),
			modelKey: usageModelKey(ctx.model),
			dailySpend: { day: 0, providerId: "anthropic", dollars: 0 },
		});
