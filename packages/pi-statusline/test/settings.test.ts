...
test("legacy Fast preferences are unknown data, not statusline configuration", () => {
	const normalized = normalizeStatuslineConfig({ codexFastMode: "legacy-value" });
	assert.equal(Object.hasOwn(normalized.config, "codexFastMode"), false);
	assert.deepEqual(
		normalized.diagnostics.map(({ code, path }) => ({ code, path })),
		[{ code: "unknown", path: "codexFastMode" }],
	);
	const root = mkdtempSync(join(tmpdir(), "pi-statusline-legacy-fast-"));
	try {
		const document = JSON.stringify({ codexFastMode: "legacy-value", palettePreset: "prism" });
		const path = join(root, "pi-statusline.json");
		saveStatuslineSettingsDocument(path, document);
