...
- **Mid-run updates** — an agent that finds something material calls `notify_parent` and keeps working. Updates enter the parent's `steer` queue immediately for its next model call, or trigger a turn if it is idle; they do not wait for the parent's run to settle. While you are blocked awaiting that agent, the update rides its own result instead
