...
// Codex catalog protocol version, not a model eligibility list. Override in pi-usage.json.
export const DEFAULT_CODEX_MODELS_CLIENT_VERSION = "0.153.4";

export interface UsageSettings {
	codexFastMode: boolean;
	codexModelsClientVersion?: string;
...
export function normalizeUsageSettings(value: unknown): UsageSettings | undefined {
	...
	if (
		Object.hasOwn(value, "codexModelsClientVersion") &&
		(typeof value.codexModelsClientVersion !== "string" ||
			!/^\d+\.\d+\.\d+$/u.test(value.codexModelsClientVersion))
	) {
		return undefined;
	}
	if (
	...
	return {
		...(typeof value.codexModelsClientVersion === "string"
			? { codexModelsClientVersion: value.codexModelsClientVersion }
			: {}),
		codexFastMode:
