...
import type { createCodexModelCatalog } from "./codex-models.js";
import {
...
type UsageExtensionDependencies = {
	...
	codexModelCatalog?: ReturnType<typeof createCodexModelCatalog>;
	sessionsDir?: string;
};
...
export default function usageExtension(
	...
	const publisher = createSnapshotPublisher(
		pi.events,
		dependencies.sessionsDir ?? join(getAgentDir(), "sessions"),
		(model) => fastRuntime.state(model),
	);
	...
	const publishStatus = (
		...
		if (
			adapterForProvider(model.provider)?.publishesStatusline === false ||
			outcome.state.status === "unsupported"
		) {
			clearStatusRefreshTimer();
			publisher.clearUsage();
			return;
		...
		if (outcome.state.status === "ready") {
			const generation = statusGeneration;
			publisher.publish(ctx, outcome.state.report, (signal) =>
				outcomeStillCurrent(ctx, model, generation, outcome, signal),
			);
		} else {
			publisher.clearUsage(outcome.state.status);
		}
		...
		try {
			...
			if (!(await outcomeStillCurrent(ctx, model, generation, outcome, controller.signal))) {
				if (sessionActive && generation === statusGeneration) {
					publisher.invalidateAccount();
					const nextModel = ctx.model;
					const nextAdapter = adapterForProvider(nextModel?.provider);
					if (nextModel && nextAdapter && nextAdapter.publishesStatusline !== false) {
						scheduleStatusRefresh(ctx, nextModel, FAILURE_BACKOFF_MS);
					}
				}
				return;
			}
	...
	const startStatusRefresh = (
		...
		void refreshCurrentStatus(ctx, model, force).catch((error) => {
			...
			publisher.clearUsage("Usage refresh failed.");
		});
	};
	...
	const showMenu = async (ctx: ExtensionCommandContext): Promise<void> => {
		...
		const menuSessionGeneration = sessionGeneration;
		const menuSessionId = ctx.sessionManager.getSessionId();
		const menuModel = ctx.model;
		statusController?.abort();
		statusController = undefined;
		clearStatusRefreshTimer();
		const controller = new AbortController();
		...
		try {
			...
			await fastRuntime.refresh(ctx, controller.signal);
			...
			const menu = defineMenu<undefined, Screen, Action, ExtensionCommandContext>({
				...
				screens: {
					main: () => {
						...
						const savedFast = settingsRuntime.get().settings.codexFastMode;
						const showFastToggle =
							fastAvailability.kind === "available" ||
							(ctx.model?.provider === "openai-codex" && savedFast);
						...
						const fastLines =
							fastAvailability.kind === "available"
								...
								: fastAvailability.kind === "unavailable" || fastAvailability.kind === "unknown"
									? [
											`Fast mode: ${fastAvailability.kind === "unknown" ? "Unknown" : "Unavailable"}${savedFast ? " (saved On)" : ""} · ${fastAvailability.reason}`,
										]
									: [];
						return {
							kind: "actions",
							...
							items: [
								...
								...(showFastToggle
									? [
											{
												id: "toggle-fast",
												label: savedFast ? "Turn Fast mode off" : "Turn Fast mode on",
												description:
				...
				actions: {
					...
					settings: async () => {
						await showUsageSettings(
							...
							() => {
								if (statusGeneration === menuGeneration && !controller.signal.aborted) {
									publisher.updateFast(ctx);
								}
							},
						);
					...
					"toggle-fast": async () => {
						...
						if (
							...
							(!savedFast && availability.kind !== "available") ||
							fastState.kind === "invalid"
						) {
							return { kind: "rejected" };
						}
						const changed = await fastRuntime.toggle(ctx, !savedFast, controller.signal);
					...
					refresh: async () => {
						...
						stableCurrent = refreshed;
						publishStableCurrent(ctx, refreshed);
						await fastRuntime.refresh(ctx, controller.signal, true);
			...
			activeControllers.delete(controller);
			// A cancelled first task never publishes a report to re-arm the timer.
			// Restore only this menu's idle owner; newer queries keep their own schedule.
			if (
				sessionActive &&
				menuSessionGeneration === sessionGeneration &&
				menuGeneration === statusGeneration &&
				!statusController &&
				!statusRefreshTimer &&
				ctx.sessionManager.getSessionId() === menuSessionId &&
				modelIdentity(ctx.model) === modelIdentity(menuModel) &&
				menuModel
			) {
				const adapter = adapterForProvider(menuModel.provider);
				if (adapter && adapter.publishesStatusline !== false) {
					scheduleStatusRefresh(ctx, menuModel);
				}
			}
		}
	};
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			...
			if (ownerGeneration === sessionGeneration && sessionActive)
	...
	fastRuntime = registerCodexFastMode(pi, settingsRuntime, (ctx) => publisher.updateFast(ctx), {
		registerSessionStart: false,
		credentialReader,
		catalog: dependencies.codexModelCatalog,
	});
}
