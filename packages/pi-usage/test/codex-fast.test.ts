...
test("account catalog capabilities, not model names, determine Fast support", () => {
	for (const id of [
		"gpt-5.4",
		"gpt-5.5",
		"gpt-5.6-luna",
		...
		assert.deepEqual(codexFastAvailability(model(id) as never, true, supported), {
			kind: "available",
			enabled: true,
		...
		assert.deepEqual(codexFastAvailability(model(id) as never, true, unsupported), {
			kind: "unavailable",
...
test("request tiers fail closed for unverified Fast but always permit standard routing", () => {
	assert.equal(codexFastRequestTier(model() as never, true, supported), "priority");
	assert.equal(codexFastRequestTier(model("gpt-6-astra") as never, true, supported), "priority");
	for (const capability of [undefined, unknown, unsupported]) {
		assert.throws(
			() => codexFastRequestTier(model() as never, true, capability),
			/Cannot request Codex Fast/,
		);
		assert.equal(codexFastRequestTier(model() as never, false, capability), "default");
	}
	assert.equal(
...
test("payload rewriting is immutable, preserves fields, and ignores foreign payloads", () => {
	...
	const rewritten = rewriteCodexFastPayload(payload, model() as never, true, supported);
	...
	assert.equal(rewriteCodexFastPayload(payload, model("gpt-6-astra") as never, true), undefined);
	for (const capability of [undefined, unknown, unsupported]) {
		assert.throws(
			() => rewriteCodexFastPayload(payload, model() as never, true, capability),
			/Cannot request Codex Fast/,
		);
	}
});

test("new Fast models retain SDK costs without guessed priority multipliers", () => {
	for (const id of ["gpt-6-astra", "new-account-model", "gpt-5.5-next"]) {
		const message = {
			role: "assistant",
			provider: "openai-codex",
			model: id,
		...
		assert.equal(codexFastAvailability(model(id) as never, true, supported).kind, "available");
		assert.equal(correctCodexFastMessageCost(message, model(id) as never, true), undefined);
...
test("priority cost correction repairs Pi's default-tier echo fallback without double charging", () => {
	...
	for (const id of ["gpt-5.6-luna", "gpt-5.6-sol", "gpt-5.6-terra"]) {
		const correctedLegacy = correctCodexFastMessageCost(
			{ ...message, model: id },
			model(id) as never,
			true,
		) as { usage: typeof usage };
		assert.equal(correctedLegacy.usage.cost.total, usage.cost.total * 2);
	}
