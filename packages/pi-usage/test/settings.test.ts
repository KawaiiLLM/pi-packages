...
test("catalog protocol version is optional, validated, and persists without enabling Fast", async () => {
	assert.equal(
		normalizeUsageSettings({ codexModelsClientVersion: "0.153.4" })?.codexModelsClientVersion,
		"0.153.4",
	);
	for (const value of [null, 42, "", "latest", "0.153", "0.153.4-dev", "../models"]) {
		assert.equal(normalizeUsageSettings({ codexModelsClientVersion: value }), undefined);
	}
	const path = await tempSettingsPath();
	const runtime = createUsageSettingsRuntime(path);
	await runtime.update({ codexModelsClientVersion: "0.154.0" });
	assert.equal(runtime.get().settings.codexFastMode, false);
	assert.deepEqual(JSON.parse(await readFile(path, "utf8")), {
		codexModelsClientVersion: "0.154.0",
	});
});
